...
export const colorPalette = [
  'blue', 'orange', 'purple', 'magenta', 'volcano', 'gold', 'geekblue', 'cyan', 'lime'
];

export const dateRangeValidate = (dateRange: any) => {
    if(!dateRange) return undefined;

    const startDate = dayjs(dateRange[0]).format(FORMATE_DATE_ENG);
    const endDate = dayjs(dateRange[1]).format(FORMATE_DATE_ENG);
...
export function convertGender(gender: "MALE" | "FEMALE" | "OTHER" | string | undefined) {
    switch(gender) {
        case "MALE":
            return "Nam";
        case "FEMALE":
